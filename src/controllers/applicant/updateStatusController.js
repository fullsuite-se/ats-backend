...
const updateStatus = async (
  ...
  try {
    ...
    if (blacklisted_type) {
      ...
      sql =
        "UPDATE ats_applicant_progress SET stage = ?, status = ?, updated_at = NOW(), blacklisted_type = ?, reason = ? WHERE progress_id = ?";
      values = [stage, converted_status, blacklisted_type, reason, progress_id];
    } else if (reason_for_rejection) {
      sql =
        "UPDATE ats_applicant_progress SET stage = ?, status = ?, updated_at = NOW(), reason_for_rejection = ? WHERE progress_id = ?";
      values = [stage, converted_status, reason_for_rejection, progress_id];
    } else {
      sql =
        "UPDATE ats_applicant_progress SET stage = ?, status = ?, updated_at = NOW() WHERE progress_id = ?";
      values = [stage, converted_status, progress_id];
    }
    ...
    if (previousStatus !== converted_status) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ${
        change_date && change_date !== "N/A" ? "?" : "NOW()"
      })
    `;
...
exports.updateApplicantStatus = async (req, res) => {
  ...
  if (isSuccess) {
    ...
    if (status.toUpperCase() === "TEST_SENT") {
      // send email
      const response = await emailController.emailTestAssessment(
        applicant_id,
        user_id
      );
      if (response == null) {
        ...
        return res.status(201).json({
          message: "Successfully updated status of applicant",
          test_assessment: "Failed to send test assessment",
        });
      }
    }
    return res
      .status(201)
      .json({ message: "Successfully updated status of applicant" });
  }
  return res
    .status(500)
    .json({ message: "Failed to update status of applicant" });
};
