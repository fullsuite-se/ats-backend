...
exports.insertIndustry = async (req, res) => {
  try {
    ...
    if (!industryName || !assessmentUrl) {
      return res.status(400).json({ success: false, message: "Missing required fields: industry name or assessment url" });
    }

    await Industry.insertIndustry(industryName, assessmentUrl, userId);
    res.status(201).json({ success: true, message: "Job Industry added successfully." });
  } catch (err) {
...
exports.updateIndustry = async (req, res) => {
  try {
    ...
    if (!jobIndId || !industryName || !assessmentUrl) {
      return res.status(400).json({ success: false, message: "Missing required fields: industry id, industry name, or assessment url" });
    }

    const updated = await Industry.updateIndustry(jobIndId, industryName, assessmentUrl);
    if (!updated) {
      return res.status(404).json({ success: false, message: "Industry not found or not updated" });
    }

    res.status(200).json({ success: true, message: "Industry updated successfully" });
  } catch (err) {
...
exports.deleteIndustry = async (req, res) => {
  try {
    ...
    if (!industry_id) {
      return res.status(400).json({ success: false, message: "Missing required field: industry id" });
    }

    const deleted = await Industry.deleteIndustry(industry_id);
    if (!deleted) {
      return res.status(404).json({ success: false, message: "Industry not found or already deleted" });
    }
